    ...
    <header className="sticky top-0 z-50 flex items-center justify-between px-6 md:px-16 py-4 border-b border-white/10 bg-[#050B1E]/80 backdrop-blur">
      ...
      {isSignedIn && (
        <nav className="hidden md:flex gap-10 text-sm text-slate-400">
          ...
          <span className="cursor-pointer hover:text-pink-400 transition">
            Automation
          </span>
        </nav>
      )}
      ...
      <div className="flex items-center gap-3">
        {!isSignedIn ? (
          ...
          <button className="px-4 py-2 text-sm rounded-lg border border-white/15 text-slate-300 hover:border-red-400 hover:text-red-400 transition">
            Logout
          </button>
        )}
      </div>
